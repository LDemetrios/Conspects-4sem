...
//        println(this@invoke + " " + args.joinToString(" "))
        it.commandLine(this@invoke, *args)
    ...
    if (res.exitValue != 0) {
        throw ExecException(
            "Command ${toOnelineCommand(listOf(this) + args)} failed " +
                    "with return code ${res.exitValue} (Probably means '${errno(res.exitValue)}')"
        )
    }

    val x = out.toString().split(Regex("\r\n|[\r\n\u2028\u2029\u0085]")).filter(String::isNotBlank)
    println("//!")
    return x
}


fun call(command: List<String>, root: File = project.projectDir): Triple<String, String, Int> {
    val out = ByteArrayOutputStream()
...
//        println(this@invoke + " " + args.joinToString(" "))
        ...
        return Triple(out.toString(), err.toString(), res.exitValue)
